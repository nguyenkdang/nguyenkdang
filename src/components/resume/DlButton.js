import React from 'react'

const dlButton = ({size}) => {
    return (
        <div>
            <svg className="dl_button" data-name="dl button" xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 321.34 321.34">
                <defs> <style>{'.cls-3 {fill: #111010;}'}</style> </defs>
                <g>
                    <path className="cls-3" d="M1655.32,507.38A160.67,160.67,0,1,0,1816,668.05,160.68,160.68,0,0,0,1655.32,507.38Zm0,288.36A127.69,127.69,0,1,1,1783,668.05,127.68,127.68,0,0,1,1655.32,795.74Z" transform="translate(-1494.65 -507.38)"/>
                    <rect className="cls-3" x="96.61" y="221.62" width="128.12" height="34.72" rx="7.38"/>
                    <path className="cls-3" d="M1710.65,652.87l-14.47-14.47a5.7,5.7,0,0,0-8.07,0l-15.43,15.43V570.09a5.24,5.24,0,0,0-5.23-5.24h-24.26a5.24,5.24,0,0,0-5.23,5.24v83.69l-15.38-15.38a5.7,5.7,0,0,0-8.07,0L1600,652.87a5.7,5.7,0,0,0,0,8.07l32.77,32.77,0,0,22.54,22.54,0,0,0,0,22.54-22.54,0,0,32.77-32.77A5.7,5.7,0,0,0,1710.65,652.87Z" transform="translate(-1494.65 -507.38)"/>
                </g>
            </svg>
        </div>
    )
}

export default dlButton
